...
<body>
            ...
            <div class="content pb-0">
                ...
                <div class="d-flex align-items-center justify-content-between gap-2 mb-4 flex-wrap">
                    ...
                    <div class="gap-2 d-flex align-items-center flex-wrap">
                        <button type="button" class="btn btn-sm btn-success action-btn btn-add-tc" data-bs-toggle="modal" data-bs-target="#editModal">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="table-actions">
                                                            <button type="button" class="btn btn-sm btn-primary action-btn btn-edit-tc" data-bs-toggle="modal" data-bs-target="#editModal"
                                                                data-id="{{ $content->id }}"
                                                                data-section-key="{{ $content->section_key }}"
                                                                data-title="{{ $content->title ?? '' }}"
                                                                data-paragraphs="{{ json_encode($content->paragraphs ?? '') }}"
                ...
                <form id="editForm" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="sectionKeySelect" class="form-label">Section Key</label>
                                    <input type="text" class="form-control" id="sectionKeySelect" style="display:none;" placeholder="Enter section key (e.g., service_agreement)" autocomplete="off">
    ...
    // ---- Add New handler ----
    document.querySelector('.btn-add-tc').addEventListener('click', function() {
        ...
        document.getElementById('footerHeading').value = '';
        document.getElementById('footerEmail').value = '';
        if (CKEDITOR.instances.paragraphs) {
            CKEDITOR.instances.paragraphs.setData('');
        }

        // Show section key text input, hide readonly display
        document.getElementById('sectionKeySelect').style.display = 'block';
        document.getElementById('sectionKeySelect').value = '';
        document.getElementById('sectionKeyDisplay').style.display = 'none';

        // Hide page meta fields by default
        document.getElementById('pageMetaFields').style.display = 'none';

        // Update modal title
        document.getElementById('editModalLabel').textContent = 'Add Terms and Conditions Content';
    });

    // When section key is typed in input (Add mode), sync to hidden input
    document.getElementById('sectionKeySelect').addEventListener('input', function() {
        document.getElementById('sectionKey').value = this.value;
    });

    // ---- Edit button handler (event delegation) ----
    document.addEventListener('click', function(e) {
        const btn = e.target.closest('.btn-edit-tc');
        if (!btn) return;

        const id = btn.dataset.id;
        const sectionKey = btn.dataset.sectionKey;
        const title = btn.dataset.title;
        const paragraphs = btn.dataset.paragraphs;
        ...
        const footerHeading = btn.dataset.footerHeading;
        const footerEmail = btn.dataset.footerEmail;
        ...
        // Show readonly display, hide select
        document.getElementById('sectionKeyDisplay').value = sectionKey;
        document.getElementById('sectionKeyDisplay').style.display = 'block';
        document.getElementById('sectionKeySelect').value = sectionKey;
        document.getElementById('sectionKeySelect').style.display = 'none';

        // Update modal title
        document.getElementById('editModalLabel').textContent = 'Edit Terms and Conditions Content';
        ...
        if (sectionKey === '_page_meta') {
            ...
            // Parse paragraphs and list items, merge into CKEditor HTML
            let paragraphsText = '';
            try {
                paragraphsText = JSON.parse(paragraphs);
            } catch (e) {
            ...
            let listItems = [];
            try {
                listItems = JSON.parse(listItemsJson);
            } catch (e) {
                listItems = [];
            }
    ...
    // ---- Form submit handler (for both Add and Edit) ----
    document.getElementById('editForm').addEventListener('submit', function(e) {
        e.preventDefault();
        
        // Sync CKEditor content back to the textarea
        if (CKEDITOR.instances.paragraphs) {
        ...
        // Determine URL based on whether we are adding or editing
        const url = id
            ? `${BASE_URL}/admin/update-terms-and-conditions-content/${id}`
            : `${BASE_URL}/admin/store-terms-and-conditions-content`;
    ...
    function deleteContent(id) {
        if (confirm('Are you sure you want to delete this content?')) {
            fetch(`${BASE_URL}/admin/delete-terms-and-conditions-content/${id}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                }
