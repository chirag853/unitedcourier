...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="content pb-0">
                ...
                <!-- Common Stats Content Table -->
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title">Common Stats Management</h5>
                                <p class="card-text">Manage the fact number statistics displayed across all website pages (About, Services, Express Air, E-commerce, Warehousing)</p>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover" id="commonStatsTable">
                                        <thead>
                                            <tr>
                                                <th>ID</th>
                                                <th>Title</th>
                                        ...
                                        <tbody>
                                            @forelse($commonStats as $stat)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="badge bg-info fs-6">{{ $stat->target_number }}</span>
                                                    </td>
                                                    <td>
                                                        <span class="badge bg-secondary">{{ $stat->suffix }}</span>
                                                    </td>
                                                    <td>{{ $stat->display_order }}</td>
                                                    <td>
                                                        <span class="badge {{ $stat->status ? 'bg-success' : 'bg-danger' }}">
                                                    ...
                                                    <td>
                                                        <div class="table-actions">
                                                            <button type="button" class="btn btn-sm btn-primary action-btn" 
                                                                ...
                                                                onclick="editStat({{ $stat->id }}, '{{ addslashes($stat->title) }}', '{{ addslashes($stat->target_number) }}', '{{ addslashes($stat->suffix) }}', {{ $stat->display_order }}, {{ $stat->status ? 'true' : 'false' }})">
                                                                <i class="ti ti-edit"></i> Edit
                                                            </button>
                                                            <button type="button" class="btn btn-sm btn-danger action-btn" onclick="deleteStat({{ $stat->id }})">
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="7" class="text-center py-4">
                                                        <p class="text-muted">No common stats found. Run the seeder to populate default data.</p>
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" id="edit_target_number" name="target_number" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Suffix</label>
                                <input type="text" class="form-control" id="edit_suffix" name="suffix" placeholder="e.g. +, K+, %, /7">
                            </div>
                        </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this stat? This action cannot be undone.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
    ...
    <script>
        ...
        // Edit stat function
        function editStat(id, title, targetNumber, suffix, displayOrder, status) {
            $('#edit_id').val(id);
            $('#edit_title').val(title);
            $('#edit_target_number').val(targetNumber);
            $('#edit_suffix').val(suffix);
            $('#edit_display_order').val(displayOrder);
            $('#edit_status').prop('checked', status === true || status === 'true' || status === 1);
            
            // Set form action
            $('#editForm').attr('action', '{{ url("/admin/update-common-stats") }}/' + id);
        }
        ...
        $('#confirmDeleteBtn').on('click', function() {
            if (deleteId) {
                $.ajax({
                    url: '{{ url("/admin/delete-common-stats") }}/' + deleteId,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
