        ...
        <div class="row g-5">
            ...
            <div class="col-lg-3 uwd-rc-sidebar-col">
                <div class="uwd-rc-sticky-nav">
                    <nav id="uwd-rc-nav-list">
                        <a href="#uwd-returns"
                            class="uwd-rc-nav-link uwd-rc-active">{{ $returnsSection->title ?? 'Returns' }}</a>
                        <a href="#uwd-cancel"
                            class="uwd-rc-nav-link">{{ $cancellationSection->title ?? 'Cancellation' }}</a>
            ...
            <div class="col-lg-9">
                <div class="uwd-rc-glass-panel">
                    <span class="uwd-rc-last-updated">Effective Date:
                        {{ $pageMeta->effective_date ?? 'October 2023' }}</span>

                    @if($returnsSection)
                    <section id="uwd-returns" class="uwd-rc-section">
                        <h2><i data-lucide="package-x"></i> {{ $returnsSection->title ?? 'Returns' }}
                        </h2>
                        <p>{!! nl2br(e($returnsSection->paragraphs ?? 'We do not accept RTOs once a shipment has been received at our hub. If the shipment is in transit to our hub and you wish to cancel, you must write to us with the shipment details or contact customer care.')) !!}</p>
                    </section>
                    @endif

                    @if($cancellationSection)
                    <section id="uwd-cancel" class="uwd-rc-section">
                        <h2><i data-lucide="x-circle"></i> {{ $cancellationSection->title ?? 'Cancellation' }}
                        </h2>
                        <p>{!! nl2br(e($cancellationSection->paragraphs ?? 'Cancellation is allowed only before the shipment is inwarded at the hub. Once inwarded, the shipment should be treated as processed, and cancellation will not apply.')) !!}</p>
                    </section>
                    @endif
                    ...
                    <div class="uwd-rc-footer-cta" id="uwd-contact">
                        ...
                        <p class="mb-0">For refund or cancellation queries, reach out to <a
                                href="mailto:{{ $pageMeta->footer_email ?? 'contact@unitedcourier.com' }}"
                                style="color: white; font-weight: 700;">{{ $pageMeta->footer_email ?? 'contact@unitedcourier.com' }}</a>
                        </p>
                    </div>
                    @endif
